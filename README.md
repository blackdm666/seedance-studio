...
已适配 [88API 当前视频规范](https://88api.ai/zh/docs/api/video/video-api-standard/)：NewAPI 任务插件继续通过 `/v1/videos` 创建与轮询。支持目录中的任务模型识别、按秒 `tiered_expr` 计费、各模型素材字段、本地图片上传和结果 URL 下载。Veo 支持 4/6/8 秒、720p/1080p 和显式图片模式；保留 Image 2.5 生图预设。

`video --dry-run` 不上传、不生成；正式创建不自动重试，查询失败可继续查询原任务。未知计价表达式会阻止付费提交。运行 `node --test tests/*.test.mjs` 执行离线回归；在线 `models --json` 与 `--self-test` 只读，不产生视频生成费用。完整变更和验证范围见 [CHANGELOG.md](CHANGELOG.md)。
