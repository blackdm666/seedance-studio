...
4. **关键帧预审（可选，便宜）。** 场景复杂、拿不准或涉及产品外观时先 `image` 生成 1 张关键帧确认方向。**只要片中有贯穿全片的主角/产品，锚定图就不是"可选"——出片前必须先生成 1 张合规锚定图并 `--ref` 锁定，防跨镜漂脸/漂形（人物一律新生成，不抽原片脸）。** 生图**默认 `gpt-image-2`**（不写 `--model` 即用它，稳定 2K；newapi 网关自带兜底，插件不自建兜底链）；**要海报级更高清再显式 `--model gpt-image-2-4k`**（16:9 真 4K UHD 3840×2160；88api 侧该渠道时有时无、断渠道直接报错、不自动回退）。**要同一角色/产品出多张一致的图**，用 `--ref <首图>` 垫图（走 `/v1/images/edits`），先出 1 张再拿它锁后续角度/场景——见 [references/api.md](references/api.md) 生图模型表。**要一次出多张不同图**（多方案锚定图 / 多分镜关键帧），重复 `--prompt` 或加 `--n <张数>`，交给并发池并行跑（`--concurrency` 默认 3、上限 10），比逐张串行快数倍；每张独立请求，单张失败不炸整批。
5. **提交生成。**
