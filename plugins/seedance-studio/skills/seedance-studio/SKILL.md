...
不要把一个模型的能力或请求体套到其它模型。插件先按内置的 88API 模型适配器确定**提交模型名、创建端点、状态端点和参数结构**，没有专用适配器时才使用实时目录的 `supported_endpoint_types` 选择统一视频端点。目录短别名只用于价格匹配，付费提交必须使用适配器给出的 88API 官方模型名。

- `veo-3.1` 价格别名 → 提交 `veo-3.1-1080p-8s`；
- `veo-3.1-fast` 价格别名 → 提交 `veo-3.1-fast-1080p-8s`；
