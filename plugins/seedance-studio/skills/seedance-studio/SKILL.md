...
4. **关键帧预审（可选，便宜）。** 场景复杂、拿不准或涉及产品外观时先 `image` 生成 1 张关键帧确认方向。生图**默认 `--model 4k`**（`gpt-image-2-4k`，16:9 出真 4K UHD 3840×2160）；**不可用/失败会自动切换 pro 模型 `gemini-3-pro-image` 重试**。**用户对画面内容或参考还原不满意时，改用 `--model gemini`（pro 模型，一致性更强）重试**。另有 `--model 2k`（便宜预审档）。**要同一角色/产品出多张一致的图**，用 `--ref <首图>` 垫图（走 `/v1/images/edits`），先出 1 张再拿它锁后续角度/场景——见 [references/api.md](references/api.md) 生图模型表。
5. **提交生成。**
