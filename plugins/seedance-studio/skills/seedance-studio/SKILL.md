...
4. **关键帧预审（可选，便宜）。** 场景复杂、拿不准或涉及产品外观时先 `image` 生成 1 张关键帧确认方向。生图有三档：`--model 2k`（默认 `gpt-image-2`，快省，做预审）/ `--model 4k`（`gpt-image-2-4k`，高清主图海报）/ `--model gemini`（`gemini-3-pro-image`，参考图一致性最强）。**要同一角色/产品出多张一致的图**，用 `--ref <首图>` 垫图（走 `/v1/images/edits`），先出 1 张再拿它锁后续角度/场景——见 [references/api.md](references/api.md) 生图模型表。
5. **提交生成。**
