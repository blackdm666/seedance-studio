...
4. **关键帧预审（可选，便宜）。** 场景复杂、拿不准或涉及产品外观时先 `image` 生成 1 张关键帧确认方向。**只要片中有贯穿全片的主角/产品，锚定图就不是"可选"——出片前必须先生成 1 张合规锚定图并 `--ref` 锁定，防跨镜漂脸/漂形（人物一律新生成，不抽原片脸）。** 生图**默认 `gpt-image-2-4k`**（不写 `--model` 即用它，16:9 出真 4K UHD 3840×2160）；**不可用/失败会自动切换 flash 快版 `gemini-3.1-flash-image` 重试**。**用户对画面内容或参考还原不满意时，改用 `--model gemini`（flash 快版，一致性强）重试**。**要同一角色/产品出多张一致的图**，用 `--ref <首图>` 垫图（走 `/v1/images/edits`），先出 1 张再拿它锁后续角度/场景——见 [references/api.md](references/api.md) 生图模型表。**要一次出多张不同图**（多方案锚定图 / 多分镜关键帧），重复 `--prompt` 或加 `--n <张数>`，交给并发池并行跑（`--concurrency` 默认 3、上限 10），比逐张串行快数倍；每张独立走兜底链，单张失败不炸整批。
5. **提交生成。**
...
- **主角先行（合规 + 一致性，默认动作不是可选）**：片中有反复出现的核心人物/产品时，出片前**先 `image` 生成一张合规「新身份」锚定图**（`--dry-run` 先看载荷），再用 `--ref`/`--image` 锁定贯穿全片——**人物一律新生成，绝不从参考视频抽帧/抽脸**（肖像权）。多形象同一张脸：先出 1 张主锚定图，再拿它当 `--ref` 出其余形象。
