...
- **上游熔断/容量类报错**（`circuit breaker` / `temporarily suspended` / `no active tokens` / `可用渠道不存在`）是 88api 上游容量问题，**非 Key/提示词/模型名问题**，失败调用不产图不计费；此类会**即时跨上游兜底**（gpt→gemini→grok，不同厂商通道），不空等上游 `auto-recovery`；全链都挂再稍后重试，别刷。
