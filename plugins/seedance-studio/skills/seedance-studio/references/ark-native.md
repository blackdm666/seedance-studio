...
> 当前 88API 客户端协议以 [api.md](api.md) 为准。本文的 Ark `content[]`、role、编辑/延长字段不是 2026-09-15 后任务插件的公开提交格式，禁止直接透传或据此承诺支持。

> 来源：火山方舟官方文档 `ark.volcengine.com/docs/82379/2607688`（2026-08 抓取）。
> **重要**：本插件从 **88api.ai** 的实时目录读取视频模型。只有用户明确选择当前目录中的 Seedance 2.5 模型时才使用本文；请求仍走 88API `/v1/videos`。本文记录的是**字节官方 Ark 原生能力**（端点 `/api/v3/contents/generations/tasks`，模型 `doubao-seedance-2-5-260628`），二者不是同一套 API。
...
| 参数 | 默认 | 可选 | 说明 |
|---|---|---|---|
| `resolution` | `720p` | `480p` / `720p` | **2.5 不支持 1080p / 4k**；插件按用户所选 88API 模型 ID 和实时目录推断本次分辨率 |
...
- 本插件不从参考视频抽取演员脸：原片演员使用新身份；用户另行提供并明确指定的已获授权真人素材则通过 `--identity-ref` / `--identity-image` 保持为唯一身份权威，与上方“已获授权的真人素材”路径一致。
