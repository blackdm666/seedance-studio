...
T2 一定是多镜头 × 多秒数 = **真金白银**。启动前**必须先立项确认**（这是 T2 唯一强制打扰用户的点，之后全自动）：
...
写 `series-bible.md`：一句话核心概念、题材调性、目标受众、集数与时长规划、总预算(秒)、风格锁定（画幅/色调/镜头语言/声音策略）。展示给用户做**唯一一次**成本确认。

## 阶段 1：剧本（免费，决定成败）

加载 [screenwriting.md](screenwriting.md) 审核/打磨剧本：**无聊是最大的罪，第一集决定生死**。产出通过审核的分集大纲与 `scripts/epNN.md`。没通过剧本审核，绝不进入烧钱的生成阶段——在错的本子上生成 = 精准地浪费钱。
